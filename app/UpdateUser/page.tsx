...
export default function UpdatePage() {
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await axios.put(process.env.NEXT_PUBLIC_DB_URL+"/user", formData, {
        headers: {
  ...
  return (
   <PrivateRoute>
     ...
     <div className='max-w-5xl block m-auto bg-white my-10 rounded-md overflow-hidden'>
      ...
      {values.img ? (<img className="w-[150px] h-[150px]" alt="img" style={{borderRadius:"50%", display:'block',margin:"auto"}} src={URL.createObjectURL(values.img)}/> ):  (<Image src={user.img as string} width={150} height={150} alt={user.name as string} style={{borderRadius:"50%", display:'block',margin:"auto"}}></Image>)}
      <input
